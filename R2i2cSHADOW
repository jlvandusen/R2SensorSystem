// =======================================================================================
//        SHADOW_MD:  Small Handheld Arduino Droid Operating Wand + MarcDuino + R2 Sensor
// =======================================================================================
//                          Last Revised Date: 10/13/2015
//                             Version 3.5
...
// =======================================================================================
#ifdef COLLISION
String Part = "shadow";                           // scan targets 1 = left, 2 = right and 3 = dome
...
bool movingfwd() { return (state == stateMovingFwd); }           // Quick methods for setting each state to true by calling the subroutine
bool movingbck() { return (state == stateMovingBck); }           // Quick methods for setting each state to true by calling the subroutine
bool turning() { return (state == stateTurning); }               // Example: moving(); or turning ();...
bool stopped() { return (state == stateStopped); }
bool playfull() { return (state == statePlayfull); }
bool waypoint() { return (state == stateWaypoint); }
...
void recenterDomeOnly(void)  {
  ...
  if ((currentTime - DomelastDecisionTime) >= automationInterval  && (domenav!=bodynav)) {
    ...
    float error = domenav - bodynav;                    // calculate which way to turn the dome to intercept the targetHeading
    ...
    if ((error = error * -1)<=HEADING_TOLERANCE) {
     ...
     SyR->motor(domeSpeed);
          ...
          Serial.print  ("Error: ");
          Serial.print  (error);
          Serial.println("...Auto Return Dome\r\n");
...
if (state==stateMovingFwd)  {
  if (distance>=SAFE_DISTANCE)  {
    if (turnDirection==straight) {
      ...
      speed = FAST_SPEED;
        ...
        #ifdef SHADOW_DEBUG
            Serial.println("Distance > SAFE Turning: SLOW and set back to Straight");
        #endif
        turnDirection = straight;
        speed = TURN_SPEED;
      }
  ...
  if (distance>=SAFE_DISTANCE)  {
    if (turnDirection==straight) {
      #ifdef SHADOW_DEBUG
          Serial.println("Distance > SAFE Straight: NORMAL Backward");
      #endif
      speed = -NORMAL_SPEED;
    } else  {
        #ifdef SHADOW_DEBUG
            Serial.println("Distance > SAFE Turning: TURN Backward");
        #endif
        turnDirection = straight;
        speed = -TURN_SPEED;
      }
...
//    if (turnDirection==straight) {
//      #ifdef SHADOW_VERBOSE
//          Serial.print("Straight > Setting Speed to FAST");
//      #endif
//      speed = FAST_SPEED;
//    } else  {
//      #ifdef SHADOW_VERBOSE
//          Serial.print("Distance > SAFE Turning: SLOW and set back to Straight");
//      #endif
//      speed = TURN_SPEED;
//    }
//     if (footDriveSpeed<=speed)
...
//      ST->turn(turnDirection * invertturnDirection);        // already turning to navigate
//      #ifdef SHADOW_VERBOSE      
//          Serial.print("Distance > safe: footSpeed: ");
...
//      ST->turn(turnDirection * invertturnDirection);        // already turning to navigate
//      #ifdef SHADOW_VERBOSE      
//          Serial.print("Distance > Turn & < safe: footSpeed: ");
//          Serial.print(footDriveSpeed);
//          Serial.print(" Direction: ");
//          Serial.print(turnDirection);
//          Serial.print(" speed: ");
...
//    ST->turn(turnDirection * invertturnDirection);
//    #ifdef SHADOW_VERBOSE      
//        Serial.print("Distance < Turn & > Stop: footdrivespeed: ");
//        Serial.print(footDriveSpeed);
//        Serial.print(" Direction: ");
//        Serial.print(turnDirection);
//        Serial.print(" speed: ");
...
//            ST->drive(footDriveSpeed);
//            #ifdef SHADOW_VERBOSE      
//                Serial.print("Stopping: footdrivespeed: ");
//                Serial.print(footDriveSpeed);
//                Serial.print(" Direction: ");
//                Serial.print(turnDirection);
//                Serial.print(" speed: ");
...
//        }
// } else if (stopped()) {
//    if ((distanceleftfront>TURN_DISTANCE) && (distancerightfront>TURN_DISTANCE) && (distanceleftside>TURN_DISTANCE) && (distancerightside>TURN_DISTANCE) && (distanceleftback>TURN_DISTANCE) && (distancerightback>TURN_DISTANCE))  {
//            turnDirection = straight;
//            ST->turn(turnDirection * invertturnDirection);      // R2 stands still
//            speed = 0; 
//        } else {
//            turnDirection = straight;
//            ST->turn(turnDirection * invertturnDirection);      // R2 stands still
...
void R2safedistance() {
  if (state==stateMovingFwd) {
    if (distanceleftfront<=SAFE_DISTANCE||distancerightfront<=SAFE_DISTANCE){
      makedecision = true;
      #ifdef SHADOW_VERBOSE   
  ...
  if (state==stateTurning) {
    if (distanceleftside<=SAFE_DISTANCE||distancerightside<=SAFE_DISTANCE){
      makedecision = true;
      #ifdef SHADOW_VERBOSE   
        Serial.println("makedecision is true ");
        Serial.print(distanceleftside);
...
//if ( abs(footDriveSpeed) > 50)
//    turnnum = (map(turnDirection, 54, 200, -(turnspeed/4), (turnspeed/4)));
//else if (turnnum <= 200 && turnnum >= 54)
//    turnnum = (map(turnDirection, 54, 200, -(turnspeed/3), (turnspeed/3)));
//else if (turnnum > 200)
//    turnnum = (map(turnDirection, 201, 255, turnspeed/3, turnspeed));
//else if (turnnum < 54)
//    turnnum = (map(turnDirection, 0, 53, -turnspeed, -(turnspeed/3)));
...
void R2soundcheck() {
  ...
  // Sound Direction Algorithm
    if (mic1>mic2 || mic1>mic3) {
     domeTargetPosition = mic1direction;
    }
    if (mic2>mic4 || mic2>mic1) {
     domeTargetPosition = mic2direction;
    }
    if (mic3>mic1 || mic3>mic4) {
     domeTargetPosition = mic3direction;
    }
    if (mic4>mic3 || mic4>mic2) {
     domeTargetPosition = mic4direction;
    }
  }
}
